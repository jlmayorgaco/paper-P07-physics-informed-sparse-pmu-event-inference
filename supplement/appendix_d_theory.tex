...
\begin{theorem}[First-order support-to-support separation]
...
\end{equation}
Thus the shared true component cancels exactly at first order. In particular,
for $S=\{i,j\}$ and $C=\{i,k\}$ the margin is independent of the amplitude at
$i$. Enlarging a competitor can only reduce the margin because its explanatory
subspace grows.
...
After nuisance and shared-source quotienting, principal angles between
$\operatorname{range}\overline{\bm F}_S$ and
$\operatorname{range}\overline{\bm F}_C$ summarize the worst aligned severity
directions. A zero angle caused only by the literal shared-source intersection is
not evidence that the exclusive source components are indistinguishable. This is
why the frozen weak-event audit residualizes the shared direction before reporting
quotient geometry.

Equations~\eqref{eq:supp-support-margin-definition}--
\eqref{eq:supp-shared-support-quotient} assume a local signed-amplitude
neighborhood. With sign, magnitude, or duration constraints, the linear-subspace
projection is a lower bound unless the constrained optimum is shown to be
interior; the constrained cone or difference set must otherwise be used.
...
Fisher projection and support geometry & Toy identities pass; IEEE 39-bus convergence to $\gamma_1$ remains untested. \\
