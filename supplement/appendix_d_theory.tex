...
The production callback changes physical parameters while preserving the stored coordinates at the callback. The DAE solver then restores consistency before the first retained frame. The relevant differentiable object is therefore the sampled map
\begin{equation}
...
Assume $\bm N$ has full column rank in a regular observable nuisance chart. If $\bm P_{\perp}\bm d\ne\bm0$, the local squared distance to the nuisance manifold is
\begin{equation}
 \Delta^2(a)=a^2\|\bm P_{\perp}\bm d\|_2^2+O(a^3).
 \label{eq:supp-first-order-contact}
\end{equation}
If $\bm d=\bm N\bm v$, define
\begin{equation}
 \bm\kappa=\bm P_{\perp}
...
At the origin, its derivative with respect to $\bm b$ is
...
$\bm q-\tfrac12\bm H_0[\bm v,\bm v]$ onto $\operatorname{range}\bm N$; the remaining normal component is \eqref{eq:supp-relative-curvature}. Squaring the residual gives \eqref{eq:supp-second-order-contact}. $\hfill\square$
...
Efficient Fisher projection & First-order amplitude information after declared nuisance is removed & Source-wise detectability association is exploratory; pairwise Fisher did not match the pooled multicandidate outcome \\
