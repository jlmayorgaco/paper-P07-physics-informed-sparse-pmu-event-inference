...
The paper-by-paper literature matrix, full AC and terminal-current conventions, detailed event operators, metric definitions, complete proofs, nuisance-profiled Fisher information, temporal resolvability limits, full estimator and likelihood equations, two stepwise algorithm contracts, all campaign contracts, calibration ablations, weak single-source development results, and the historical known-source benchmark now appear in `supplement.tex`. The executed 137-hypothesis algorithm and the unexecuted joint design are deliberately separated. The main paper retains only the evidence needed to change the reader's scientific conclusion.
