...
Eleven deterministic checks are retained in `scripts/verify_theory_identities.py` and `generated/theory_checks.json`. They guard the branch primitive; regular first-order and shared-tangent second-order laws; rank-deficient profiling; shared-source cancellation; competitor and nuisance monotonicity; nested-support degeneracy; the principal-angle identity; diagnostic-quotient factorization; and nuisance-span reparameterization invariance. A generated supplementary table summarizes them without presenting toy systems as power-system evidence.
