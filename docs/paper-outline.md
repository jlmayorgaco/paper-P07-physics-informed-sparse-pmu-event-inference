...
5. **Consequence:** A successful system could reconstruct the hidden network while declaring when the available PMUs support a source decision; the current evidence establishes the state component and a 16-source load-intervention special case, but not the multi-family end-to-end result.
...
| VIII. Mismatch and Event Results | Calibration/mismatch/adaptation, load detectability and source-resolution limits, then the known-source baseline and source-transfer failure. |
