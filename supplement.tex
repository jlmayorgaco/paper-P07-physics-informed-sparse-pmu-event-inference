...
\begin{abstract}
This supplement records the detailed literature matrix, electrical conventions, mathematical proofs, Fisher-information analysis, Bayesian estimator construction, frozen experimental contracts, and secondary results supporting the main article. It distinguishes the independently evaluated state component and implemented batch event estimator $\mathcal E_{\mathrm B}$ from the proposed joint Hybrid-DAE estimator $\mathcal E_{\mathrm J}$. The latter remains unexecuted. This supplement does not add empirical claims or change any test denominator. Every empirical table and figure is generated from the same hashed evidence snapshot used by the main manuscript; separate deterministic tables audit stated algebraic identities.
\end{abstract}
\begin{keywords}
...
\begin{equation}
 f_i^{\mathrm{phys}}=f_0+\frac{1}{2\pi}\frac{d\angle V_i}{dt},\qquad
 \operatorname{ROCOF}_i^{\mathrm{phys}}=\frac{1}{2\pi}\frac{d^2\angle V_i}{dt^2},
\end{equation}
but an operational likelihood must reproduce the causal filtering of the PMU backend.

The preceding equations define the ideal physical quantities $f_i^{\mathrm{phys}}$ and $\operatorname{ROCOF}_i^{\mathrm{phys}}$ implied by the network trajectory. Reported quantities $f_i^{\mathrm{meas}}$ and $\operatorname{ROCOF}_i^{\mathrm{meas}}$, however, are outputs of a dynamic frequency-estimation process and can exhibit disturbance- and estimator-dependent transient error and latency. A prior benchmark by the authors compared 16 dynamic frequency estimators spanning six practical groups over 34 disturbance scenarios and quantified these trade-offs under ramps, phase jumps, harmonics, ringdown, and composite IBR disturbances \cite{Mayorga2026FrequencyBenchmark}. We therefore distinguish
\begin{equation}
 \begin{bmatrix}f_i^{\mathrm{meas}}\\\operatorname{ROCOF}_i^{\mathrm{meas}}\end{bmatrix}
 =
 \begin{bmatrix}f_i^{\mathrm{phys}}\\\operatorname{ROCOF}_i^{\mathrm{phys}}\end{bmatrix}
 +\bm\varepsilon_{fr,i},\qquad
 \operatorname{Cov}(\bm\varepsilon_{fr,i})=\bm\Omega_{fr}.
\end{equation}
Because phasor, frequency, and ROCOF outputs may be estimated from overlapping underlying waveform or phasor information, their errors need not be conditionally independent. An augmented likelihood therefore permits cross-covariance
\begin{equation}
 \bm\Omega_{\mathrm{aug}}=
 \begin{bmatrix}
 \bm\Omega_{VI}&\bm\Omega_{VI,fr}\\
 \bm\Omega_{fr,VI}&\bm\Omega_{fr}
 \end{bmatrix};
\end{equation}
neither the magnitude of $\bm\Omega_{VI,fr}$ nor a block-diagonal approximation is assumed without calibration. Frequency and ROCOF are retained as candidate channels for an augmented sparse-PMU likelihood. Their incremental source-discrimination value, covariance structure, and effect on localization delay are not assumed here and require the dedicated $VI$, $VI+f$, $VI+\operatorname{ROCOF}$, and $VI+f+\operatorname{ROCOF}$ ablation.
...
\FloatBarrier
\input{supplement/appendix_f_single_event_pilot}
