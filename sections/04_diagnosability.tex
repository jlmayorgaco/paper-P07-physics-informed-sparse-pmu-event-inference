...
after nuisance removal. For true support $S$, severity direction
...
\end{equation}
The competitor is allowed to refit its amplitudes; comparing columns at equal
amplitude would overstate separation. Shared sources cancel from the discriminant
at first order, leaving only the true-exclusive component after projection onto
nuisance, shared, and competitor-exclusive directions. If $S\subset C$ and an
extra amplitude may approach zero, the geometric infimum is zero. Geometry can
rank non-nested explanations, but cardinality among nested supports requires a
model-selection penalty or integrated evidence. Supplementary Appendix~D proves
these statements and gives the associated principal-angle interpretation.
