...
This paper & Hidden voltage field; state uncertainty & Eight-label ontology; all load supports with $K\le2$ & DAE hypotheses; quadratic physical likelihood & Eight prescribed PMU buses & Physical bank avoids source-label head; learned baseline fails holdout & Yes \\
\bottomrule
\end{tabularx}
\end{table*}
...
The available evidence covers only part of this intersection. PowerDynamics supports hidden-voltage estimation, physical recentering, and prospective zero-, one-, and two-source load inference. A retrospective audit links weak-case recovery to profiled manifold distance, while a 120-frame diagnostic studies information accumulation without upgrading it to a recovery experiment. ANDES supports known-source event recognition and documents failure under source holdout. Multi-family candidate-conditioned inference with state and integrity nuisance still requires one common experimental contract.
