...
- Network and operating-point mismatch dominate the tested estimator shifts. A held-out, measurement-only static physical MAP closes 97.4--98.3% of the frozen M6 operating-point error gap, although the nuisance information matrix has rank 25 of 43.
...
- Under the stated local linear-Gaussian assumptions, coincident source signatures after projection away from initial-state nuisance are indistinguishable; no classifier can overcome that information limit.
- Adding a conditionally independent, correctly modeled PMU block cannot decrease the pairwise Gaussian information. This is not a guarantee for an arbitrary learned classifier.
