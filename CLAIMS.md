...
- The implemented event estimator is the batch physical-manifold Bayesian bank $\mathcal E_{\mathrm B}$. It fixes operating point, onset, and load family; compares 137 supports with the frozen quadratic response manifold and W2 covariance; and marginalizes one- or two-dimensional amplitudes by GH31 quadrature. A sequential implementation is the same estimator only if it reproduces the batch posterior on every prefix, including the initial AR(1) density.
- The second proposal is the joint Hybrid-DAE estimator $\mathcal E_{\mathrm J}$ over state, operating nuisance, event family, onset, support, amplitude, and integrity. It is a specified but unexecuted statistical object. Under restriction $\mathcal R_0$, where every prior, candidate, covariance, and surrogate is made identical to $\mathcal E_{\mathrm B}$, it must reduce to $\mathcal E_{\mathrm B}$; this is a design regression condition, not an empirical result.
- The paper defines physical interventions and measurement-integrity states as separate, composable latent variables.
- Candidate-conditioned bus currents, branch-terminal currents, complex powers, and model-implied frequency quantities are deterministic functionals of the state and network mode proposed for $\mathcal E_{\mathrm J}$; they are not outputs of $\mathcal E_{\mathrm B}$ or independent learned targets. Only the hidden complex-voltage target has current empirical reconstruction evidence from the separate state component.
...
- In the common-nuisance whitened local model, every profiled event likelihood ratio factors through the range of the nuisance-residualized event dictionary. Its rank is an intrinsic linear diagnostic dimension, not a PMU count or a global nonlinear observability certificate.
